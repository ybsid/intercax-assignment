...
public class DeployedController {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DeployedController.class);
    private static final DateTimeFormatter pattern = DateTimeFormatter.ofPattern("yyyy-MM-dd");
    @GetMapping("/getAll")
    public List<Robots> getAllDeployed(){
        LOGGER.info("returning all designed Robots");
        return repository.getDeployed();

    }

    @GetMapping("/getCount")
    public int getAllDeployedCount(){
        LOGGER.info("returning all tested count");
        return repository.getDeployedCount();
    }
